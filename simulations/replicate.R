generator <- function(npop = 10, sd = 5, holdout=0.5, ncells = 1000, ndim = 5) {
    force(npop)
    force(ndim)
    force(sd)
    force(ncells)
    function() {
        ...
        keep <- !logical(ncells)
        keep[sample(ncells, ncells * holdout)] <- FALSE
...
dir <- "results"
write.table(df, file=file.path(dir, "replicate.tsv"), row.names=FALSE, sep="\t")
